...
const toDbM  = (f,uid,tid) => ({ titolo:f.titolo, tipo:f.tipo||"ordinaria", stato:f.stato, priorita:f.priorita||"media", operatore_id:f.operatoreId?Number(f.operatoreId):null, cliente_id:f.clienteId?Number(f.clienteId):null, asset_id:f.assetId?Number(f.assetId):null, piano_id:f.pianoId?Number(f.pianoId):null, data:f.data, durata:Number(f.durata)||60, note:f.note||"", user_id:uid, ...(f.sottotipo&&{sottotipo:f.sottotipo}), ...(f.causaGuasto&&{causa_guasto:f.causaGuasto}), ...(f.fermoImpianto!=null&&{fermo_impianto:!!f.fermoImpianto}), ...(f.downtimeOre&&{downtime_ore:Number(f.downtimeOre)}), ...(tid&&{tenant_id:tid}) });
        ...
        {vista==="richieste" && (
          isCliente
            ...
            : <GestioneRichieste
                ...
                tenantId={tenant?.id}
                meOperatore={meOperatore}
                onApprova={approvaRichiesta}
                onRifiuta={rifiutaRichiesta}
              />
        )}
